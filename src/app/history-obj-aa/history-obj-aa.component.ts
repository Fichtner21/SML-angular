...
import { FetchMatchesService } from '../history-obj/fetch-matches.service';

@Component({
  selector: 'app-history-obj-aa',
  templateUrl: './history-obj-aa.component.html',
  styleUrls: ['./history-obj-aa.component.scss']
})
export class HistoryObjAaComponent implements OnInit {
  ...
  constructor(private activatedRoute: ActivatedRoute, private router: Router, private httpClient: HttpClient, private fetchMatches: FetchMatchesService, private tabApiService: PlayersApiService, private translateService: TranslateService, private db: AngularFireDatabase) {
  ...
  ngOnInit() {
    ...
    this.playersTab$ = this.tabApiService.getPlayers('Players_AA').pipe(
      map((response: any) => {
    ...
    this.matchesTab$ = this.tabApiService.getPlayers('Match+History+AA').pipe(
      map((response: any) => {
  ...
  getTitle(playerName: any): string {
    if (playerName.ban === 'TRUE') {
      return 'Banned';
    } else if (playerName.active === 'FALSE') {
      return 'Inactive';
    } else {
      return 'Active';
    }
